...
if (!empty($_GET['search'])) {
    $data = $_GET['search'];
    $sql = "SELECT * FROM chamado WHERE id_chamado LIKE '%$data%' or descricao LIKE '%$data%'  ORDER BY id_chamado DESC";
} else {
...
while ($chamado_data = mysqli_fetch_assoc($result)) //enquanto chamada_data receber o result e retornar matriz associativa
{
    ...
    if ($chamado_data['status'] == 1) {
     ...
     echo "<td>
        ...
        <a href='delete.php?id=$chamado_data[id_chamado]' title='Deletar'>
     ...
     echo "<td>
        <a href='concluido.php?status=$chamado_data[id_chamado]'>
